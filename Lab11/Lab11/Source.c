...
#define MAX_LIST (3) // 3 umisto 11 jer imamo samo 7 drzava pa da to bar na nesto lici
#define MAX_ROW (64)
#define MAX_WORD (32)
...
typedef struct _Node {
	...
	// cvorovi sadrze korijen stabla, ono sta u main izgleda kao:
	// Branch root; ili BranchPosition root=malloc(sizeof(Branch));
} Node;
...
int addSortedIntoHashTable(NodePosition head, NodePosition country) {
	...
	for (temp = head->next; temp != NULL; temp = temp->next) { // temp = head->next; while(temp != NULL) {...; temp = temp->next;}
		if (strcmp(country->country_name, temp->country_name) < 0) {
			previousNode = findPrevioustNode(head, temp);
			country->next = previousNode->next; // ili country->next = temp;
			previousNode->next = country;
			return 1;
		}
	}
	// izasli smo iz petlje jer nema temp koji zadovoljava uvjet u IF statementu
